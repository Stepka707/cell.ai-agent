...
| Glucose | 100 mg/mL |
| NaCl | 5 M |
| MgSO₄ | 100 mM (working stock, prediluted from 1 M) |
| KCl | 200 mM (working stock, prediluted from 2 M) |
| (NH₄)₂SO₄ | 1 M |
| K₂HPO₄ | 1 M |
| KH₂PO₄ | 100 mM (working stock, prediluted from 1 M) |
| MOPS pH 7 | 1 M |
| CaCl₂ | 100 mM (working stock, prediluted from 1 M) |
| Trace metals | 100× (working stock, prediluted from 1000×) |
| Sodium Citrate | 100 mM (working stock, prediluted from 1 M) |
| Sodium L-Glutamate | 1 M |
| Iron(II) Sulfate Heptahydrate | 10 mM (working stock, prediluted from 1 M) |
...
- For every stock addition, compute: `volume_µL = (final_conc_mM × total_volume_µL) ÷ stock_conc_mM`
- If the result is below 5 µL, increase the total well volume incrementally until the addition reaches 5 µL, up to a maximum of **6000 µL**
- If the addition still cannot reach 5 µL at 6000 µL total volume, discard the formulation
- When increasing `total_volume`, recalculate all other components
...
Each formulation consists of one base medium plus additions from the available stocks, with a **target total volume of ~2000 µL** (adjustable up to 6000 µL if required by pipetting constraints).

The search space includes both base medium choice and additive composition — each formulation should represent your best prediction for maximizing growth rate, using a distinct combination of base medium and additives.
...
- All formulations must respect the operational constraints defined above
- Verify every constraint explicitly before finalizing each formulation
- Your reasoning will be logged and compared against a parallel Bayesian optimizer post-hoc

**Output two files:**
1. A markdown file with all reasoning, constraint checks, and calculations
2. A JSON file with the formulation data in this schema: `id`, `name`, `base_medium`, `base_medium_volume_uL`, `total_volume_uL`, and `components` (array of `name`, `stock_concentration`, `volume_uL`, `final_concentration`)
