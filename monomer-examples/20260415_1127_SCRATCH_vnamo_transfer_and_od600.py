...
OVERVIEW
--------
1. Transfer 200 µL from VNAMO_CELL_STOCK_002 well A1 (with pre-mix 3x175uL) into
...
NOTE ON MIX RATE
----------------
Mix rate (flow rate) is not exposed in the transfer_array schema. It is hardcoded
in the Opentrons Python protocol. To control it, ask the Monomer team to expose
it as a routine parameter.
"""
...
def build_definition(
    ...
    :param volume_ul: Transfer volume in uL (default 200).
    :return: Complete workflow definition.
    """
    workflow = WorkflowDefinitionDescriptor(
        description=(
            f"Transfer {volume_ul} uL from {cell_stock_barcode} {src_well} "
            f"to {plate_barcode} {dst_well} (premix 3x175uL), then read OD600."
        )
    )
